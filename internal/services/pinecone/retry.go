...
func (r *RetryService) RetryWithTimeout(call func(ctx context.Context) error) error {
    ...
    return NewRetryError("retry_exhausted", "operation failed after all retries", lastErr)
}

func NewRetryError(operation, msg string, cause error) *PineconeError {
    return &PineconeError{Type: ErrTypeRetry, Operation: operation, Message: msg, Cause: cause}
}
