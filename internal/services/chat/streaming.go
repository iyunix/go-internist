// G:\go_internist\internal\services\chat\streaming.go
package chat
...
func (s *StreamingService) StreamChatResponse(
    ...
    // Save user message
    if err := s.saveUserMessage(ctx, chatID, prompt); err != nil {
        return NewRAGError("save_message", "failed to save user message", err)
    }
